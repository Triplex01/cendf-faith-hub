...
const Hero = () => {
  ...
  return <section id="accueil" className="relative h-[50vh] md:h-[60vh] min-h-[350px] md:min-h-[450px] max-h-[450px] md:max-h-[550px] flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0">
        {heroSlides.map((slide, index) => <img key={index} src={slide.image} alt={slide.alt} className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ${index === currentSlide ? "opacity-100" : "opacity-0"}`} />)}
        <div className="absolute inset-0 bg-gradient-to-b from-primary/80 via-primary/50 to-background/90" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_transparent_0%,_rgba(0,0,0,0.4)_100%)]" />
      </div>

      {/* Carousel Navigation */}
      <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-background/10 backdrop-blur-sm border border-gold/30 flex items-center justify-center text-gold hover:bg-gold/20 transition-all" aria-label="Image précédente">
        <ChevronLeft className="w-5 h-5" />
      </button>
      <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-background/10 backdrop-blur-sm border border-gold/30 flex items-center justify-center text-gold hover:bg-gold/20 transition-all" aria-label="Image suivante">
        <ChevronRight className="w-5 h-5" />
      </button>

      {/* Dots */}
      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2">
        {heroSlides.map((_, index) => <button key={index} onClick={() => setCurrentSlide(index)} className={`h-2 rounded-full transition-all duration-300 ${index === currentSlide ? "w-8 bg-gold" : "w-2 bg-primary-foreground/40 hover:bg-primary-foreground/60"}`} aria-label={`Diapositive ${index + 1}`} />)}
      </div>
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gold/20 backdrop-blur-sm border border-gold/40 mb-6 animate-fade-in">
            ...
            <span className="text-xs font-semibold text-gold uppercase tracking-wider">
              ​Commission Épiscopale pour la Doctrine de la Foi en Côte d'Ivoire
          ...
          {/* Scripture Quote */}
          <p className="font-secondary text-base md:text-lg text-primary-foreground/90 max-w-xl mx-auto mb-6 leading-relaxed animate-slide-up italic" style={{
          animationDelay: "0.1s"
        }}>
          ...
          animationDelay: "0.2s"
            ...
            <Button variant="gold" size="lg" onClick={toggle} className="group relative overflow-hidden shadow-lg shadow-secondary/50 animate-pulse-soft hover:shadow-xl hover:shadow-secondary/60 transition-all duration-300">
