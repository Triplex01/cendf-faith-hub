...
const RadioPlayer = ({ variant = "full", className }: RadioPlayerProps) => {
  const {
    isPlaying,
    isLoading,
    volume,
    ...
    return (
      <div className={cn("flex items-center gap-3", className)}>
        <button
          onClick={toggle}
          disabled={isLoading}
          className="w-10 h-10 rounded-full bg-primary flex items-center justify-center hover:bg-primary/90 transition-all"
        >
          {isLoading ? (
  ...
  if (variant === "compact") {
    return (
      <div className={cn("bg-card rounded-xl p-4 border border-border shadow-card", className)}>
        <div className="flex items-center gap-4">
          <button
            onClick={toggle}
            disabled={isLoading}
            className="w-14 h-14 rounded-full bg-gradient-burgundy flex items-center justify-center hover:shadow-burgundy transition-all flex-shrink-0"
          >
            {isLoading ? (
  ...
  // Full variant
  return (
    <div className={cn("bg-gradient-to-br from-primary to-primary-glow rounded-2xl p-8 shadow-elegant text-primary-foreground", className)}>
